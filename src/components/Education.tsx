...
const Education = () => {
  ...
  return (
    <section id="education" className="bg-[hsl(var(--cream-warm))] py-16 sm:py-24 lg:py-32">
      <div className="mx-auto max-w-7xl px-5 sm:px-6 lg:px-12">
        <div className="mb-10 sm:mb-16 grid gap-6 sm:gap-8 lg:grid-cols-2 lg:items-end">
          <div>
            <p className="eyebrow mb-4 sm:mb-6">Education</p>
        ...
        <div className="grid gap-6 sm:gap-8 lg:grid-cols-[1.1fr_1fr]">
          <div className="space-y-5 sm:space-y-6">
            {items.map((e, i) => (
              <article key={e.degree} className="editorial-card bg-card p-6 sm:p-8 lg:p-10">
                <div className="flex items-start justify-between gap-4 sm:gap-6">
                  <div className="flex gap-4 sm:gap-5">
                    <div className="flex h-10 w-10 sm:h-12 sm:w-12 flex-shrink-0 items-center justify-center bg-foreground text-background">
                      <GraduationCap className="h-5 w-5 sm:h-6 sm:w-6" />
                    </div>
                    <div>
                      <p className="eyebrow mb-2">{e.year} · {e.location}</p>
                      <h3 className="font-playfair text-xl sm:text-2xl text-foreground">{e.degree}</h3>
                      <p className="mt-1 text-sm text-foreground/70">{e.institution}</p>
                      <p className="mt-3 sm:mt-4 text-sm leading-relaxed text-foreground/70">{e.description}</p>
                    </div>
                  </div>
                  <span className="shrink-0 font-playfair text-2xl sm:text-3xl text-foreground/30">0{i + 1}</span>
                </div>
              </article>
            ))}
          </div>

          <aside className="ink-block p-6 sm:p-8 lg:p-10">
            <p className="text-[10px] uppercase tracking-[0.22em] text-white/60">Focus areas</p>
            <h3 className="mt-4 font-playfair text-2xl sm:text-3xl leading-tight">
              What I studied &amp; keep sharpening today
            </h3>
            <div className="hairline my-6 sm:my-8 !bg-white/20" />
            <ul className="grid gap-y-3 gap-x-6 sm:grid-cols-2">
              {focus.map((f) => (
                <li key={f} className="flex items-center gap-3 text-sm text-white/85">
                  <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-[hsl(var(--coral))]" /> {f}
                </li>
              ))}
            </ul>
          </aside>
